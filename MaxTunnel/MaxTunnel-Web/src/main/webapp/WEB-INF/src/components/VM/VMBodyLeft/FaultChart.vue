<template>
      <div class="FaultChartContainer BackgroundImage">
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            drawRadar(){
            let _this=this;
            _this.myChart = _this.$echarts.init(document.getElementById(_this.radarChartid));
              let option = {
               title: {
            ...
            fetchData(requestUrl,callback){
              ...
              _this.axios.get(requestUrl).then(result=>{
                ...
                if(code==200){
                    let baseData=callback.call(this.fetchData,data,this.recursion);

                    if(JSON.stringify(baseData.indicator)!=JSON.stringify(_this.indicator)
                       ||JSON.stringify(baseData.initObj)!=JSON.stringify(_this.initObj)){
            ...
            recursion(data,initObj,indicator){
                for(let index=0;index<data.length;index++){
                    ...
                    if(typeof item.val =='object'){
                      ...
                      indicator.push({name:item.key,max:item.val.reduce((maxVal,curVal)=>curVal.val>maxVal?curVal.val:maxVal,0)})
                      continue;
                    }
