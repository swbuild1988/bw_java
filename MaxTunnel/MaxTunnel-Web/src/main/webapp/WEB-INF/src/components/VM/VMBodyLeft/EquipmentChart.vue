...
<script>
  ...
  export default {
      ...
      methods:{
          ...
          drawBar(){
              let _this=this;
              _this.myChart = _this.$echarts.init(document.getElementById(_this.barSimpleid));
              let option={
                textStyle: {
                ...
                if(code==200){
                    let newData=data.reduce((init,item)=>{
                        return{
                            ...
                            sData:init.sData==undefined?[].concat(item.val):[].concat(init.sData,item.val)
                        }
                    },{})
                    if(JSON.stringify(newData.xData)!=JSON.stringify(_this.xData)||JSON.stringify(newData.sData)!=JSON.stringify(_this.xData)){
