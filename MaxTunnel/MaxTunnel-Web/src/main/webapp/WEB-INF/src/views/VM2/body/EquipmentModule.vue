<template>
    <div class="Main">
        ...
        <div class="bottomBox">
            <stack-chart v-bind="stackCrossData" ref="stackCrossChart"></stack-chart>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            init() {
                // 获取数据
                EquipmentService.getEquipmentTypeAndStatusCount()
                    .then(res => {
                        // 柱状图数据
                        var arr = [];
                        ...
                        for (var i = 0; i < res.length; i++) {
                            ...
                            for (var j = 0; j < res[i].val.length; j++) {
                                if (i == 0) {
                                    this.stackCrossData.legendData.push(
                                        res[i].val[j].key
                                    );
                        ...
                        for (var i = 0; i < arr.length; i++) {
                            this.stackCrossData.seriesData.push({
                                data: arr[i]
                            });
                        }
                        ...
                        for (
                            var i = 0; i < this.stackCrossData.legendData.length; i++
                        ) {
                            this.pieData.legendData.push(
                                this.stackCrossData.legendData[i]
                            );
                        }
                        ...
                        for (var i = 0; i < arr.length; i++) {
                            ...
                            for (var j = 0; j < arr[i].length; j++)
                                this.pieData.seriesData[i] += arr[i][j];
                        }
                        ...
                        for (
                            let i = 0, pieData = this.pieData; i < pieData.seriesData.length; i++
                        ) {
            ...
            percentage(array, index) {
                ...
                return Math.round((array[index] / sum) * 1000) / 10 + "%";
            },
            refreshData() {
                setInterval(() => {
...
<style scoped>
    ...
    .stateLists>li {
        ...
        border-image: linear-gradient(0deg,
                transparent 0%,
                rgba(22, 78, 137, 0.5) 30%,
                rgba(22, 78, 137, 1) 50%,
                rgba(22, 78, 137, 0.5) 80%,
                transparent 100%) 1;
    }
