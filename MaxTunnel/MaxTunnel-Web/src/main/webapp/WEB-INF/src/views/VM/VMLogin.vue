      ...
      handleSubmit (name) {
        ...
        _this.$refs[name].validate((valid) => {
          if (valid) {
            _this.logining = true;
            _this.loading=true;
            _this.formValidate.checked?_this.setCookie(this.formValidate.userName,this.formValidate.passWord,7):_this.clearCookie();
            ...
            _this.axios.post('/login',loginParams).then(result => {
              ...
              if (code !== "200") {
                ...
                sessionStorage.setItem('VMUser', JSON.stringify(data));
...
<style scoped>
  .login-container {
    ...
    background: rgba(3,23,56,.4);
    border: 1px solid rgba(166,218,241,1);
    box-shadow: 0 0 4em rgba(60,166,234,.8);
    position: absolute;
    left: 50%;
    top: 50%;
  ...
  .main >>>　.ivu-input-group-append,.main>>> .ivu-input-group-prepend{
    color: #fff;
