        ...
        <Row>
            <Col span="12">
                <FormItem label="合同名称：" prop="name">
                    <Input v-model="contractInfo.name" placeholder="请输入合同名称" v-if="!read" class="inputWidth"></Input>
                    <span v-if="read">{{ contractInfo.name }}</span>
                </FormItem>
                <FormItem label="合同开始日期：" prop="contractStartTime">
                    <DatePicker type="datetime" placeholder="请选择合同开始日期" v-model="contractInfo.contractStartTime" class="inputWidth" v-if="!read"></DatePicker>
                    <span v-if="read">{{ new Date(contractInfo.contractStartTime).format('yyyy-MM-dd hh-mm-s') }}</span>
                </FormItem>
                <FormItem label="合同结束日期：" :prop="read ? null : 'contractEndTime'"> 
                    <DatePicker type="datetime" placeholder="请选择合同结束日期" v-model="contractInfo.contractEndTime" class="inputWidth" v-if="!read"></DatePicker>
                    <span v-if="read">{{ new Date(contractInfo.contractEndTime).format('yyyy-MM-dd hh-mm-s') }}</span>
                </FormItem>
                <FormItem label="付款方式：" prop="payType">
                    ...
                    <span v-if="read && item.val == contractInfo.payType" v-for="(item,index) in payTypes" :key="index">{{ item.key }}</span>
                </FormItem>
                <FormItem label="合同状态：" v-if="pageType==pageTypes.Edit || read">
                    ...
                    <span v-if="read && item.val == contractInfo.contractStatus" v-for="(item,index) in contractStatus" :key="index">{{ item.key }}</span>
                </FormItem>
                <FormItem label="管线名称：" prop="cableName">
                    <Input v-model="contractInfo.cableName" placeholder="请输入管线名称" v-if="!read" class="inputWidth" />
                    <span v-else>{{ contractInfo.cableName }}</span>
                </FormItem>
                <FormItem label="管线长度：" prop="cableLength">
                    <Input v-model="contractInfo.cableLength" placeholder="请输入管线长度" v-if="!read" class="inputWidth" />
                ...
                <FormItem label="管线状态：" v-if="pageType==pageTypes.Edit || read">
                    ...
                    <span v-if="read && item.val == contractInfo.cableStatus" v-for="(item,index) in cableStatus" :key="index">{{ item.key }}</span>
                </FormItem>
                <FormItem label="录入人：">
                    <Input v-model="editor.name" v-if="!read" class="inputWidth" readonly/>
                    <span v-else>{{ contractInfo.editorName }}</span>
                </FormItem>
                <FormItem label="企业：" :prop="read ? null : 'companyId'">
                    <Poptip placement="top">
                        <Input v-model="customerName" placeholder="请选择企业" class="inputWidth" v-if="!read" readonly />
                        <div class="pop" slot="content" v-show="pageType != pageTypes.Read">
                            <customer-choose @selectCustomer="getcompanyId" v-bind:companyId="contractInfo.companyId"></customer-choose>
                        </div>
                    </Poptip>
                ...
                <FormItem label="管线位置：">
                    <Row style="width: 45vmin;margin-left: 14vmin">
                        <Col span="8">
                            ...
                            <span v-if="read && item.id == contractInfo.tunnelId" v-for="(item,index) in cableLocation.tunnels" :key="index">{{ item.name }}</span>
                        ...
                        <Col span="8">
                            ...
                            <span v-if="read && item.id == contractInfo.storeId" v-for="item in cableLocation.stores" :key="item.id">{{ item.name }}</span>
                            <div class="ivu-form-item-error-tip location" v-if="checkCable.store">请选择仓</div>
                        </Col>
                        <Col span="8">
                        <Select multiple v-model="contractInfo.areaIds" @on-change="check('area')" v-if="!read">
                                <Option v-for="(item,index) in cableLocation.areas" :value="item.id" :key="index">{{ item.name }}</Option>
                            </Select>
                            <p v-if="read && contractInfo.areaIds.indexOf(item.id) != -1" v-for="(item,index) in cableLocation.areas" :key="index">{{ item.name }}</p>
            ...
            <Col span="12">
                <FormItem label="合同文件:" prop="file">
                    <Input placeholder="支持doc,pdf,txt,wps,docx,dotx格式" type="file" @on-change="fileChange" class="inputWidth"
                    accept="application/msword,application/pdf,text/plain,application/vnd.ms-works,
                ...
                <FormItem>
                    <div style="width: 44vmin; height: 42vmin;margin-left: 14vmin;overflow: auto">{{file}}</div>
                </FormItem> 
            </Col>
        </Row>
        <div style="text-align: center">
            <Button type="primary" @click="submitcontractInfo('contractInfo')" v-if="pageType!=pageTypes.Edit && pageType != pageTypes.Read">提交</Button>
            <Button type="primary" @click="submitEditContractInfo('contractInfo')" v-if="pageType==pageTypes.Edit">更新</Button>
            <Button type="ghost"  @click="handleReset('contractInfo')" style="margin-left: 8px" v-if="pageType != pageTypes.Read">取消</Button>
            <Button type="ghost"  @click="handleReset('contractInfo')" style="margin-left: 8px" v-if="read">返回</Button>
        </div>
    </Form>
    ...
    methods:{
        getInitData() {
            ...
            this.axios.get('roles/users').then(res=>{
                ...
                if(code==200){
                    this.editor = data[0]
                    this.contractInfo.editorId = data[0].id
                }
            })
        },
        submitcontractInfo(name){
            this.$refs[name].validate((valid)=>{
                let check = this.contractInfo.companyId && this.contractInfo.tunnelId 
                && this.contractInfo.areaIds && this.contractInfo.storeId
                if(valid && check){
                    if(new Date(this.contractInfo.contractStartTime)>new Date(this.contractInfo.contractEndTime)){
                        this.$Message.error('开始时间必须小于结束时间！');
                        return;
                    } else {
                        let _this = this
                        ContractService.addContract(_this.contractInfo).then(
                            (result)=>{
        ...
        getParams(){
            if(this.pageType == this.pageTypes.Edit || this.pageType == this.pageTypes.Read){
                let _this = this
                ContractService.getDetailsByContractId(_this.contractInfo.id).then((result)=>{
                    _this.contractInfo.name = result.cableDto.contract.name;
                    _this.contractInfo.contractStartTime = new Date(result.cableDto.contract.contractStartTime);
                    _this.contractInfo.contractEndTime = new Date(result.cableDto.contract.contractEndTime);
                    _this.contractInfo.payType = result.cableDto.contract.payType.toString();
                    _this.contractInfo.companyId = result.cableDto.contract.company.id;
                    _this.contractInfo.cableName = result.cableDto.cableName;
                    _this.contractInfo.cableLength = result.cableDto.cableLength.toString();
                    // _this.contractInfo.sectionIds = result.sectionIds;
                    _this.customerName = result.cableDto.contract.company.name.toString();
                    _this.contractInfo.cableStatus = result.cableDto.cableStatus.toString();
                    _this.contractInfo.contractStatus = result.cableDto.contract.contractStatus.toString();
                    _this.contractInfo.cableId = result.cableDto.id;
                    _this.contractInfo.tunnelId = result.tunnelId;
        ...
        submitEditContractInfo(name){
            this.$refs[name].validate((valid)=>{
                ...
                if(valid && check){
                    ...
                    let _this = this
                    ContractService.updateContract(_this.contractInfo).then(
                        (result)=>{
                            _this.Log.info('success')
                            _this.$nextTick(()=>{
                                _this.$router.push("/UM/tunnelContract/list")
        ...
        fileChange(e){
            this.contractInfo.file = e.srcElement.files[0].name
            let reader = new FileReader();
            reader.readAsText(e.srcElement.files[0],"gb2312");
            let _this = this
            reader.onload = function(f){
                _this.file = f.target.result
            }
        }
    }
}
