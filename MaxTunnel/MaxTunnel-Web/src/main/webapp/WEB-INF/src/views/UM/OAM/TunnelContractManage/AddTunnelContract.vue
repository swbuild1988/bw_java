<template>
	<div :style="backStyle">
		...
		<Row>
			...
			<FormItem>
				<pdf :page="file.curPage" :class="['pdfContent',{'pdfOverflow': file.src}]" @num-pages="file.totalPage=$event"
				@page-loaded="file.curPage=$event" @loaded="loadPdfHandler" :src="file.src"></pdf>
				<div style="margin-left: 20vmin;">
					<Button type="primary" @click="changePdfPage(0)" size="small" v-show="file.src && file.curPage > 1">上一页</Button>
					<Button type="primary" @click="changePdfPage(1)" size="small" style="float:right;margin: 0.4vmin 5vmin" 
					v-show="file.src && file.curPage < file.totalPage">下一页</Button>
				</div>
			</FormItem>
			</Col>
		</Row>
...
export default {
	data() {
		return {
			...
			ruleValidate: {
				...
				path: [{ required: true, message: "请选择合同文件", trigger: "blur" }]
			},
			customerName: "",
			sectionNames: "",
			tunnels: [],
	...
	methods: {
		...
		submitcontractInfo(name) {
			this.$refs[name].validate(valid => {
				...
				if (
					...
					ContractService.uploadFile(this.file.uploadParam, config).then(
						res => {
		...
		if (
			...
			let _this = this;
			let url = '/'+_this.ApiUrl.split('/')[3] + '/contracts/' + _this.contractInfo.id + '/view'
			this.file.src = pdf.createLoadingTask({ url: url });
			ContractService.getDetailsByContractId(_this.contractInfo.id).then(
				result => {
					_this.file.editInit = true
					_this.contractInfo = {
						name: result.cableDto.contract.name,
						contractStartTime: new Date(result.cableDto.contract.contractStartTime),
						contractEndTime:  new Date(result.cableDto.contract.contractEndTime),
						...
						operateUsername: result.cableDto.contract.operateUsername,
