<template>
	<div class="answers">
		...
		<div v-if="loginData.isLogin">
			...
			<div class="list">
				<Table border ref="selection" :columns="columns" :data="inboundList"></Table>
			</div>
		    <!-- <Page :total="page.pageTotal" :current="page.pageNum" show-total
		        placement="top" @on-change="handlePage" show-elevator
		        class="pageStyle">
		    </Page> -->
		</div>
	</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			search(){
				let _this = this
				CommunicationService.getInbound(this.token,this.inboundid).then(
					result=>{
						_this.inboundList = []
						_this.inboundList = result.inbound
					},
					error=>{
