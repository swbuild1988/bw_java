    ...
    <div>
        ...
        <Row class="list">
            ...
            <Col span="12">
                  ...
                  <!-- <simple-line-chart style="width: 42vw;height: 36vh;" v-bind="lineChart"></simple-line-chart> -->
            </Col>
        </Row>
    </div>
</template>
  ...
  export default {
    ...
    data() {
      return {
          ...
          lineChartData: {
              legendData: ['风机类电表','灯类电表','水泵类电表','其他类电表'],
              xAxisData: ['周一','周二','周三','周四','周五','周六','周日'],
              data: [
                  {
                      name:'风机类电表',
    ...
    components: { pileBarChart,SimpleLineChart },
    mounted() {
        // this.getData()
        this.drawChart()
    },
    ...
    methods: {
        getData() {
            ...
            this.axios.post('energies/'+ queryTime +'/datagrid',param).then(res=>{
                let { code,data } = res.data
                this.data = [] 
                if(this.query.cycleType == 2){
                    this.data = data.pagedList
                    this.data.forEach(time=>{
                        time.date = new Date(time.date).format("yyyy-MM-dd")
                    })
                  }else{
                      data.pagedList.forEach(time=>{
                          ...
                          temp.date = new Date(time.start).format('yyyy-MM-dd') +' ~ '+new Date(time.end).format('yyyy-MM-dd')
                          temp.val = time.val
                          this.data.push(temp)
                      })
                  }
            ...
            this.axios.get('energies/'+queryTime+'/'+startTime).then(res=>{
                let { code,data } = res.data
                ...
                data[0].data.forEach(time=>{
                    ...
                    if(this.query.cycleType == 2){
                        ...
                        date = new Date(time.start).format("yyyy-MM-dd")+' ~ '+ new Date(time.end).format('yyyy-MM-dd')
                    }
            ...
            this.axios.get('energies/'+queryTime+'01/'+startTime).then(res=>{
                let { code,data } = res.data
                ...
                if(this.query.cycleType == 2){
                    ...
                    data[0].data.filter(time=>{
                        let date = new Date(time.start).format('yyyy-MM-dd') +' ~ ' + new Date(time.end).format('yyyy-MM-dd')
                        this.lineChartData.xAxisData.push(date)
                    })
                }
        ...
        drawChart() {
            this.lineChart = this.$echarts.init(document.getElementById('eCReportLine'))
