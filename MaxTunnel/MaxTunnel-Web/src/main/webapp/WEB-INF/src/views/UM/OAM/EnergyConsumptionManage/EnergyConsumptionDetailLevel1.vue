<!--管廊能耗总览层级1-->
<template>
    <div style="text-align: center;display: flex;height: 100%;justify-content: center;">
        <span style="font-size: 4vmin;color: #fff;align-self: center;">因设备不完整，功能尚未开放</span>
        <!-- <Row class="top">
    ...
    methods: {
        ...
        changeDateType(index) {
            ...
            if (index == 1) {
                ...
                _this.chartPrams.endTime = new Date().format(
                    "yyyy-MM-dd hh:mm:ss"
                );
                _this.isReady = true;
            } else if (index == 2) {
        ...
        queryDetail() {
            var _this = this;
            if (
                new Date(_this.chartPrams.startTime) >
                new Date(_this.chartPrams.endTime)
            ) {
                _this.$Message.error("开始时间必须小于结束时间！");
                return;
            }
            _this.clickBarChartByStore.clickPrams.tunnelId = parseInt(
                _this.$route.params.id
            );
            _this.clickBarChartByStore.clickPrams.energyType = _this.chartPrams
                .ammeterType
                ? 1
                : 2;
            _this.clickBarChartByStore.clickPrams.startTime = new Date(
                _this.chartPrams.startTime
            ).getTime();
            _this.clickBarChartByStore.clickPrams.endTime = new Date(
                _this.chartPrams.endTime
            ).getTime();
