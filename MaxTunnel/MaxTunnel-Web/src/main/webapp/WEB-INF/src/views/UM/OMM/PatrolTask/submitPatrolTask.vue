<template>
    <div class="formBG">
            ...
            <Form ref="task" :model="task" :rules="taskRules" :label-width="140" ENCTYPE="multipart/form-data">
                ...
                <FormItem label="任务开始时间：" prop="startTime" class="required">
                    <DatePicker type="datetime" v-model="task.startTime" placeholder="请输入巡检任务开始时间" style="width: 100%"
                        :readonly="this.$route.params.isFinished==true"></DatePicker>
                </FormItem>
                ...
                <FormItem label="巡检记录：">
                    ...
                    <Button type="dashed" long @click="handleAddRecords" icon="plus-round"
                        v-show="this.$route.params.isFinished!=true" style="color: #fff">添加巡检记录</Button>
                ...
                <FormItem label="巡检图片">
                    ...
                    <Upload v-show="this.$route.params.isFinished!=true" ref="upload" :show-upload-list="false"
                        :default-file-list="defaultList" :on-success="handleSuccess"
                        :format="['jpg','jpeg','png','gif']" :max-size="2048" :on-format-error="handleFormatError"
                        :on-exceeded-size="handleMaxSize" multiple type="drag" :action=action
                ...
                <FormItem label="巡检描述：">
                    <Input v-model="task.describe" type="textarea" :rows="4" placeholder="请输入巡检描述"
                        :readonly="this.$route.params.isFinished==true"></Input>
                </FormItem>
        ...
        <Modal v-model="showAddDefect" title="添加缺陷" :width="modalWidth" @on-ok="ok">
            ...
            <table class="addRecords" v-for="(item,index) in addRecords" :key="index" v-if="item.status==1">
                ...
                <tr>
                    ...
                    <td style="width: 15%">
                        <DatePicker type="datetime" v-model="item.recordTime" placeholder="请收入巡检记录时间"
                    ...
                    <td>
                        <Checkbox v-model="item.hasDefect" class="records" style="text-align: center;"
                            @on-change="addDefect(index)">是否发现故障</Checkbox>
                    </td>
                    <td>
                        <Button type="error" @click="handleRecord(index)" class="records"
                            :disabled="index==0">删除</Button>
                    </td>
                </tr>
                        ...
                        <table class="adddefect">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td style="width: 15%">
                                        <DatePicker type="datetime" v-model="item.defect.createTime"
                                            placeholder="请输入发现时间" style="width: 100%"></DatePicker>
                                    </td>
                                    <td>
                                        <Select v-model="item.defect.type">
                                            <Option v-for="(item,index) in type" :key="index" :value="item.val">
                                                {{item.key}}</Option>
                                        </Select>
                                    </td>
                                    <td v-show="item.defect.type==2">
                                        <Select v-model="item.defect.objectId">
                                            <Option v-for="(item,index) in objs" :key="index" :value="item.key">
                                                {{item.key}}</Option>
                                        </Select>
                                    </td>
                                    <td>
                                        <Select v-model="item.defect.level">
                                            <Option v-for="(item,index) in defectLevel" :key="index" :value="item.val">
                                                {{item.key}}</Option>
                                        </Select>
                                    </td>
                                    <td style="width: 25%">
                                        <Input v-model="item.defect.description" type="textarea"
                                            placeholder="请输入缺陷描述"></Input>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
        ...
        mounted() {
            ...
            PatrolService.getTDetailByPlanId(this.task.id).then(
                (result) => {
                    ...
                    //获取所属区域
                    TunnelService.getAreasByTunnelId(_this.defectTunnelId).then(
                        (result) => {
                            _this.areas = result
                        },
                        (error) => {
                    ...
                    //获取所属仓段
                    TunnelService.getStoresByTunnelId(_this.defectTunnelId).then(
                        (result) => {
                            _this.stores = result
                        },
                        (error) => {
