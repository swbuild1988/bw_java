...
export default {
    ...
    data(){
        return{
            ...
            stacChart: {
                ...
                xData:[],
                legendData:[
                    '未完成巡检计划','已完成巡检计划','未完成巡检任务','已完成巡检任务','未解决缺陷','已解决缺陷'
                ]
            },
            tunnels:[],
            dataTypes:[
            ...
            column:[
                ...
                },
                {
                    title: '位置',
                    render:(h,params)=>{
                        let temp = params.row.tunnelName+params.row.area.name+params.row.store.name
                        return h('span',temp)
                    }
                },
                {
            ...
            tableData:[
                {id:1,name:'test',tunnelName:'古城一街',area:{id:1,name:'一区'},store:{id:1,name: '燃气仓'},createTime:new Date().getTime(),num: 10}
            ]
        }
    },
    watch(){
    ...
    methods:{
        ...
        changeCycle(index) {
            ...
            switch (index) {
                case 1: {
                    ...
                    let startTime = new Date(date.valueOf() - 1 * 24 * 60 * 60 * 1000);
                    let result = startTime.format("yyyy-MM-dd hh:mm:ss") + '-' + date.format("yyyy-MM-dd hh:mm:ss") ;
                    this.conditions.time = result;
                    break;
                }
                case 2: {
                    this.conditions.readonly = true;
                    let startTime = new Date(date.valueOf() - 7 * 24 * 60 * 60 * 1000);
                    let result = startTime.format("yyyy-MM-dd hh:mm:ss") + '-' + date.format("yyyy-MM-dd hh:mm:ss") ;
                    this.conditions.time = result;
                ...
                case 3: {
                    this.conditions.readonly = true;
                    let startTime = new Date(date.valueOf() - 30 * 24 * 60 * 60 * 1000);
                    let result = startTime.format("yyyy-MM-dd hh:mm:ss") + '-' + date.format("yyyy-MM-dd hh:mm:ss") ;
                    this.conditions.time = result;
                    break;
                }
                case 4: {
                    this.conditions.readonly = true;
                    let startTime = new Date(date.valueOf() - 365 * 24 * 60 * 60 * 1000);
                    let result = startTime.format("yyyy-MM-dd hh:mm:ss") + '-' + date.format("yyyy-MM-dd hh:mm:ss") ;
        ...
        getWeekTime(start,end,k,days){
            ...
            for(var i=1; i<=k; i++){
                if(start<=end){
                    var time = new Date(new Date(start).valueOf() + days*(24 * 60 * 60 * 1000)*i)
                    var weekDay
                    if(k==7){
