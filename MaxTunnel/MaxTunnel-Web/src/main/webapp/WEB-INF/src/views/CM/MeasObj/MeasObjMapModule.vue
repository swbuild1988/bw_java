        ...
        <Modal v-model="show.state" title="新增流程节点" width=600>
            <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="120">
                <FormItem label="监测对象Id：" prop="objectId">
                    <Input v-model="formValidate.objectId" placeholder="请输入监测对象名称" class="InputWidth"/>
                </FormItem>
                <FormItem label="输入值：">
                    <Select v-model="formValidate.inputValue" class="InputWidth">
                        <Option v-for="item in lists" :key="item.val" :value="item.val">{{ item.key }}</Option>
                    </Select>
                </FormItem>
                <FormItem label="MaxView监测对象Id：" prop="objectId2">
                    <Input v-model="formValidate.objectId2" placeholder="请输入MaxView监测对象" class="InputWidth"/>
                </FormItem>
                <FormItem label="输出值：">
...
export default {
    ...
    data(){
        return {
            ...
            ruleValidate:{
                objectId: [
                    { required: true, message: '监测对象不能为空', trigger: 'blur' }
                ],
                objectId2: [
                    { required: true, message: 'MaxView监测对象不能为空', trigger: 'blur' }
                ]
            },
            lists: []
        }
