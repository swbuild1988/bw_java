...
export default {
    ...
    PatrolService.getPatrolCounts().then(
      result => {
        ...
        _this.currMonthRatio = Number(
          ((result.monthPlanCount / result.yearPlanCount) * 100).toFixed(2)
        );
        _this.finishedYearPlanRatio = Number(
          ((result.finishTaskCount / result.yearTaskCount) * 100).toFixed(2)
        );
      },
      error => {
  ...
  methods: {
    ...
    getRadius2() {
      if (
        this.$refs.circleBox2.offsetWidth >= this.$refs.circleBox2.offsetHeight
      ) {
