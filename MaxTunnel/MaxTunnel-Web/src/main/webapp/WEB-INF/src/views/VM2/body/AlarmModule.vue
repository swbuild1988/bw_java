    ...
    <div class="main">
        ...
        <div class="total">
            <div class="subTitle">
                ...
                <!-- <span class="yearIcon">
                    <Icon :type="yearTotal.isRise ? 'arrow-up-c' : 'arrow-down-c'"
            ...
            <div class="subTitle">
                ...
                <!-- <span class="monthIcon">
                    <Icon :type="monthTotal.isRise ? 'arrow-up-c' : 'arrow-down-c'"
        ...
        mounted() {
            ...
            if (this.carouselInfo.isCarousel) {
                this.carouselInfo.intervalId = setInterval(() => {
                    _this.carousel();
                }, parseFloat(this.carouselInfo.time));
            }
        },
        methods: {
            getTotalAlarmCount() {
                AlarmService.getYearAndMonthAlarmCount()
                    .then(result => {
                        result.forEach(item => {
                            if (item.key == "year") {
                ...
                AlarmService.getNewAlarmsList()
                    .then(result => {
                        if (result) {
                            ...
                            this.Log.info("getNewAlarmsList", result)
                            this.alarmAllData.forEach(alarm => {
                                alarm.alarmLevel = alarm.alarmLevel.slice(0, 2);
                                alarm.alarmDate = new Date(alarm.alarmDate).format(
                                    "MM-dd  hh:mm"
                                );
