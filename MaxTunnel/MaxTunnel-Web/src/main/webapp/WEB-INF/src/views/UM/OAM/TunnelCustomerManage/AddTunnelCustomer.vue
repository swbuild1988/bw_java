    ...
    <Form ref="companyInfo" :model="companyInfo" :label-width="140">
        ...
        <Row>
            <Col span="7">
                ...
                <FormItem label="信用代码:" prop="company.creditNo"
                ...
                <FormItem label="注册地址:" prop="company.address"
                :rules="{required: true, message: '请输入地址', trigger: 'blur'}">
                    <Input v-model="companyInfo.company.address" placeholder="请输入注册地址信息" />
                </FormItem>
                <FormItem label="注册电话:" prop="company.phone"
                :rules="[{required: true, message: '请输入注册电话', trigger: 'blur'},{ validator: validates.validatePhone, trigger: 'blur' },
                    { validator: validateTel, trigger: 'blur' }]">
                    <Input v-model="companyInfo.company.phone" placeholder="请输入注册电话" />
                </FormItem>
                <FormItem label="邮箱:" prop="company.mail"
            ...
            <Col span="7" offset="1">
                ...
                <FormItem label="手机:" prop="list[0].tel"
                ...
                <FormItem label="姓名:" prop="list[1].contact"
                ...
                <FormItem label="手机:" prop="list[1].tel"
                :rules="[{required: true, message: '请输入一般联系人手机', trigger: 'blur'},{ validator: validates.validatePhone, trigger: 'blur' }]">
                    <Input v-model="companyInfo.list[1].tel" placeholder="请输入紧急联系人手机" />
                </FormItem>
                <FormItem label="电话:" prop="list[1].tel2"
                :rules="{ validator: validatePhone, trigger: 'blur' }">
            ...
            <Col span="8" offset="1" class="inspectionItems">
                ...
                <FormItem label="时间:" prop="company.inspectionTime"
                ...
                <FormItem label="身份证号:" prop="company.insIdentity"
                :rules="{required: true, message: '请输入身份证', trigger: 'blur'}">
                    <Input v-model="companyInfo.company.insIdentity" placeholder="请输入固定巡检人员身份证号" />
                </FormItem>
                <FormItem label="职责:" prop="company.insDuty"
                :rules="{required: true, message: '请输入职责', trigger: 'blur'}">
                    <Input v-model="companyInfo.company.insDuty" placeholder="请输入固定巡检人员职责" />
                </FormItem>
                <FormItem label="手机：" prop="company.insPhone"
                ...
                <FormItem label="资格证类型:" prop="company.insQualification"
                :rules="{required: true, message: '请输入资格证类型', trigger: 'blur'}">
                    <Input v-model="companyInfo.company.insQualification" placeholder="请输入固定巡检人员资格证类型" />
                </FormItem>
                <FormItem label="编号:" prop="company.qualificationNo"
                :rules="{required: true, message: '请输入资格证编号', trigger: 'blur'}">
                    <Input v-model="companyInfo.company.qualificationNo" placeholder="请输入资格证编号" />
                </FormItem>
                <FormItem label="证书图片:" prop="company.imgUrl"
...
export default {
    ...
    computed:{
        inspectionInfo(){
            ...
            return  company.insName + '-' + company.insIdentity + '-' +
            company.insDuty + '-' + company.insPhone + '-' + company.insQualification + '-' +
    ...
    methods:{
                ...
                if(valid){
                    ...
                    switch(this.pageType){
                        case this.pageTypes.Add:
                            CompanyService.addCompany(this.companyInfo).then(
                                response=>{
                                    ...
                                    if(response.id){
                                        CompanyService.uploadQualification(response.id,this.uploadImageParam,config).then(
                                            res=>{
                        ...
                        case this.pageTypes.Edit:
                            Promise.all([CompanyService.editCompany(this.companyInfo),
                            CompanyService.uploadQualification(this.companyInfo.company.id,this.uploadImageParam,config)]).then(
                                response=>{
        ...
        getParams(){
            if(this.pageType==this.pageTypes.Edit){
                CompanyService.getcompanyDetail(this.companyInfo.company.id).then(
                    response=>{
        ...
        imgChange(e){
            ...
            this.companyInfo.company.imgUrl = window.URL.createObjectURL(e.srcElement.files[0])
