...
<template>
    <div style="padding: 5px;">
                        ...
                        <Col span="12">
                            <div class="datapanle boxBG">
                                ...
                                <div>
                                    <ul>
                                        <li
                                            class="takeBackUpList"
                                            v-for="(item,index) in backupTakeData"
                                            :key="index"
                                        >
                                            ...
                                            >
                                                ...
                                                <span
                                                    class="orderDetails"
                                                    @click="showTakingDetails(item.id)"
                                                >&nbsp;&nbsp;详情</span>
                                            </span>
                                        </li>
                                    </ul>
                            ...
                            <div class="datapanle boxBG">
                                ...
                                <div>
                                    <ul class="takeBackUpUl">
                                        <li
                                            class="takeBackUpList"
                                            v-for="(item,index) in takeUserData"
                                            :key="index"
                                        >
                                            ...
                                            >
                                                ...
                                                <span
                                                    class="orderDetails"
                                                    @click="showTakingPersonDetails(item.id)"
                                                >&nbsp;&nbsp;详情</span>
                                            </span>
                                        </li>
                                    </ul>
            ...
            <Col span="12" class="boxBG backListTable" style="margin-top: 1.5vh; padding: 1vmin;">
                <Row class="queryCondition">
                    ...
                    <Col span="8">
                        ...
                        <DatePicker
                            type="datetime"
                            v-model="outStorageConditions.startTime"
                            placeholder="请输入开始时间"
                            style="width: 60%"
                        ></DatePicker>
                    </Col>
                    <Col span="8">
                        ...
                        <DatePicker
                            type="datetime"
                            v-model="outStorageConditions.endTime"
                            placeholder="请输入结束时间"
                            style="width: 60%"
                        ></DatePicker>
                    </Col>
        ...
        <Modal v-model="isTaking" title="取用备品详情" :width="modalWidth" @on-cancel="resetVal">
            <Row style="margin-bottom: 10px;">
                <div style="display: none;">
                    typeId:
                    <Input v-model="typeId"></Input>
                </div>
                <Col span="14">
                    <span>出库时间：</span>
                    <DatePicker
                        type="datetime"
                        ref="takingStartTime"
                        v-model="takingSpareModalConditions.startTime"
                        placeholder="请选择开始时间"
                        class="timeWidth"
                    ></DatePicker>-
                    <DatePicker
                        type="datetime"
                        ref="takingEndTime"
                        v-model="takingSpareModalConditions.endTime"
                        placeholder="请选择结束时间"
                        class="timeWidth"
                    ></DatePicker>
                </Col>
                <Col span="7">
                    ...
                    <Select
                        ...
                        ref="staffId"
                        style="width: 60%;"
                        v-model="takingSpareModalConditions.staffId"
                    >
                        <Option value="null" key="0">所有</Option>
            ...
            <div class="pageContainer">
                <Page
                    :total="takingPage.pageTotal"
                    :current="takingPage.pageNum"
                    :page-size="takingPage.pageSize"
                    show-elevator
                    show-total
                    @on-change="takingSpareHandlePage"
                    @on-page-size-change="takingSpareHandlePageSize"
                ></Page>
            </div>
            <div slot="footer">
        ...
        >
            <Row style="margin-bottom: 10px">
                ...
                <Col span="14">
                    ...
                    <DatePicker
                        type="datetime"
                        ref="takingPersonStartTime"
                        v-model="takingPersonConditions.startTime"
                        placeholder="请选择开始时间"
                        class="timeWidth"
                    ></DatePicker>-
                    <DatePicker
                        type="datetime"
                        ref="takingPersonEndTime"
                        v-model="takingPersonConditions.endTime"
                        placeholder="请选择结束时间"
                        class="timeWidth"
                    ></DatePicker>
                </Col>
                <Col span="7">
                    ...
                    <Input
                        ...
                        type="text"
                        class="borrowName"
                        v-model="takingPersonConditions.name"
                    ></Input>
                </Col>
                <Col span="3" style="text-align: center">
            ...
            <div class="pageContainer">
                <Page
                    :total="takingPersonPage.pageTotal"
                    :current="takingPersonPage.pageNum"
                    :page-size="takingPersonPage.pageSize"
                    show-elevator
                    show-total
                    @on-change="takingPersonHandlePage"
                    @on-page-size-change="takingPersonHandlePageSize"
                ></Page>
            </div>
            <div slot="footer">
...
export default {
    ...
    methods: {
            ...
            EquipmentService.backUpHistory(this.outStorageParams).then(
                result => {
                    for (let index in result.pagedList) {
                        result.pagedList[index].inTime = new Date(
                            result.pagedList[index].inTime
                        ).format("yyyy-MM-dd hh:mm:ss");
                        result.pagedList[index].outTime = new Date(
                            result.pagedList[index].outTime
                        ).format("yyyy-MM-dd hh:mm:ss");
                    }
        ...
        getModalWidth() {
            ...
            this.tableHeight =
                this.$refs.MultiBarChartBox.offsetHeight -
                (document.body.offsetHeight / 100) * 15;
        }
    }
};
</script>
...
.backListTable .ivu-table-wrapper >>> .ivu-table:before,
.ivu-table-wrapper >>> .ivu-table:after {
    background-color: #fffdfd00 !important;
}
.backListTable .ivu-table-wrapper >>> .ivu-table th,
.ivu-table-wrapper >>> .ivu-table td {
    background-color: #fffdfd00 !important;
    border-bottom: 1px solid #7d7d7d;
}
.backListTable .pageBox .ivu-page >>> .ivu-page-total,
.backListTable .ivu-page >>> .ivu-page-options-elevator {
    color: #fff;
}
.backListTable .ivu-table-wrapper >>> .ivu-table-border td,
.backListTable .ivu-table-wrapper >>> .ivu-table-border th {
    border-right: none;
}
.ivu-select,
