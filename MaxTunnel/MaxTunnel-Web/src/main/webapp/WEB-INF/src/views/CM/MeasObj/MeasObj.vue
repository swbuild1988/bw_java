<template>
    <div>
        ...
        <div style="margin:20px;">
            <Table border stripe ref="selection" :columns="columns7" :data="data6" @on-selection-change="startdelete">
            </Table>
        ...
        <Modal v-model="editActivedModal.isShow" :title="editActivedModal.title" @on-cancel="handleReset()"
            ...
            <Form ref="unitParam" :model="editActivedModal.info" :label-width="120">
                <FormItem label="修改对象所处管廊">
                    <Select v-model="editActivedModal.info.tunnelId"
                        @on-change='editActivedModalTunnelChange(editActivedModal.info.tunnelId)'>
                        <Option v-for="item in editActivedModal.tunnels" :key="item.id" :value="item.id">{{item.name}}
                        </Option>
                    </Select>
                </FormItem>
                <FormItem label="修改对象所处区域">
                    <Select v-model="editActivedModal.info.areaId">
                        <Option v-for="item in editActivedModal.areas" :key="item.id" :value="item.id">{{item.name}}
                        </Option>
                    </Select>
                </FormItem>
                <FormItem label="修改对象所处管仓">
                    <Select v-model="editActivedModal.info.storeId">
                        <Option v-for="item in editActivedModal.stores" :key="item.id" :value="item.id">{{item.name}}
                        </Option>
                    </Select>
                </FormItem>
                <FormItem label="是否激活">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            init() {
                ...
                TunnelService.getTunnels().then(
                    result => {
                        ...
                        _this.editActivedModal.tunnels = [{
                            id: 0,
                            name: "全部"
                        }]
                        _this.editActivedModal.tunnels.push(...result)
                    },
                    error => {
                        _this.Log.info(error);
                    }
            ...
            editActivedModalTunnelChange(tunnelId) {
                let _this = this
                if (tunnelId === 0) {
                    _this.editActivedModal.stores = [{
                        id: 0,
                        name: "全部"
                    }]
                    _this.editActivedModal.areas = [{
                        id: 0,
                        name: "全部"
                    }]
                    return
                }
                TunnelService.getStoresByTunnelId(tunnelId).then(res => {
                    _this.editActivedModal.stores = [{
                        id: 0,
                        name: "全部"
                    }]
                    _this.editActivedModal.stores.push(...res)
                }, error => {
                    _this.Log.info(error);
                })
                TunnelService.getAreasByTunnelId(tunnelId).then(res => {
                    _this.editActivedModal.areas = [{
                        id: 0,
                        name: "全部"
                    }]
                    _this.editActivedModal.areas.push(...res)
                }, error => {
                    _this.Log.info(error);
                })
            },
            ...
            editActivedSave() {
                ...
                MeasObjServer.updateMeasObjsActived(tunnelId, areaId, storeId, actived).then(res => {
                    _this.$Message.success('修改成功')
                    _this.editActivedModal.isShow = false
                    _this.editActivedModal.info = {
                        tunnelId: 0,
                        areaId: 0,
                        storeId: 0,
                        actived: false
            ...
            handleReset() {
                editActivedModal.isShow = false
                editActivedModal.info = {
                    tunnelId: 0,
                    areaId: 0,
                    storeId: 0,
                    actived: false
