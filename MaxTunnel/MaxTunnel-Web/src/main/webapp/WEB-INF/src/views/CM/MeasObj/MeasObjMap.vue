<template>
    <div>
        ...
        <Modal v-model="batchAdd.isShow" title="批量添加" @on-ok="addMaps">
            <Form ref="batchAddObjMap" :model="batchAdd" :label-width="120" :rules="batchAddRules">
                <FormItem label="监测对象：" prop="objtypeId">
                    <Select v-model="batchAdd.objtypeId" class="addWidth">
                        <Option
                            v-for="item in batchAdd.list"
                            :key="item.val"
                            :value="item.val+''"
                        >{{ item.key }}</Option>
                    </Select>
                </FormItem>
                <FormItem label="开-输出值：" prop="openValue">
                ...
                <FormItem label="关-输出点位：" prop="closePosition">
...
export default {
    ...
    methods: {
            ...
            MeasObjServer.measObjMapDatagird(this.searchParams).then(
                result => {
                    ...
                    _this.tableData.forEach(data => {
                        data.crtTime = new Date(data.crtTime).format(
                            "yyyy-MM-dd hh:mm:s"
                        );
        ...
        edit(id) {
            ...
            MeasObjServer.getObjMapById(id).then(
                result => {
                    ...
                    _this.map.editInfo.objectId =
                        _this.map.editInfo.objectId + "";
                    _this.map.editInfo.objectId2 =
                        _this.map.editInfo.objectId2 + "";
                    _this.map.type = "edit";
                    _this.map.show.state = !_this.map.show.state;
                },
        ...
        addMaps() {
            ...
            this.$refs.batchAddObjMap.validate(valid => {
                if (valid) {
                    MeasObjServer.batchAddMap(params).then(
                        result => {
                            if (result) {
                                _this.$Message.info(result);
                            } else {
