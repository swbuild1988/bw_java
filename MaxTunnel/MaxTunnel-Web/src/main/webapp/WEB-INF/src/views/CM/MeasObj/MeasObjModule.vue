    ...
    <div>
        <Modal v-model="show.state" :title="title" width=600>
            <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="140">
                ...
                <FormItem label="关联预案：">
                    <Select v-model="relaPrePlan" multiple placeholder="请选择关联预案" class="InputWidth" @on-change="getTypeOf()">
                        <Option v-for="item in planIds" :value="item.val" :key="item.val">{{item.key}}</Option>
                    </Select>
                </FormItem>
                <FormItem label="管廊位置：">
                    <Row class="InputWidth">
                        ...
                        <Col span="8">
                            <Select v-model="formValidate.storeId" placeholder="请选择管仓" @on-change="getVideos">
                                <Option v-for="item in stores" :value="item.id" :key="item.id">{{item.name}}</Option>
                            </Select>
                        </Col>
                        <Col span="8">
                            <Select v-model="formValidate.areaId" placeholder="请选择区域" @on-change="getVideos">
                ...
                <FormItem label="关联视频：">
                    <Input @on-focus="changeIsShow" class="InputWidth" :disabled=readonlyVideo v-model="formValidate.videoIds"></Input>
                </FormItem>
                <FormItem label="是否激活：">
...
export default {
    ...
    methods:{
        ...
        //获取视频
        getVideos(){
            if(this.formValidate.storeId!=null && this.formValidate.areaId!=null){
                MeasObjServer.getSectionVideos(this.formValidate.storeId,this.formValidate.areaId).then(
                    result => {
                        if(result.length!=0){
