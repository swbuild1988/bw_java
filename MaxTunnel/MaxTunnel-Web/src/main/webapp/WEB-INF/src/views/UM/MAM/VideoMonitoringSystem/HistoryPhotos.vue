<template>
    <div class="allDiv">
        <div class="conditions">
            <Row>
                ...
                <Col span="3">
                    ...
                    <Button type="error" @click="downLoadPhotoes">下载</Button>
        ...
        <div class="page">
            <Page :total="page.pageTotal" :current="page.pageNum" :page-size="page.pageSize" show-sizer :page-size-opts=[8,16]
            show-total placement="top" @on-change="handlePage" @on-page-size-change='handlePageSize' show-elevator :style="pageStyle">
            </Page>
        </div>
    </div>
</template>
                        ...
                        if(this.downLoad.checkAll){
                            VideoService.allHistoryPhotoes(this.allPhotoesQuery).then(
                                res=>{
                                    ...
                                    let zip = new JSZip()
                                    let blogTitle = '历史照片'+ new Date(Date.now()).format('yyyy-MM-ddhh:m:ss');
                            ...
                            this.photos.map(photo=>{
                                if(photo.checked){
                                    ...
                                    let format = photo.path.split('.')[length - 1];
                                    let time = photo.time.replace(/ /g, '');
                                    a.download = photo.tunnel + photo.area+ photo.store + photo.name + time + '.' +format; 
                                    let url = '/'+ this.ApiUrl.split('/')[3] + '/snaps/download/' + photo.path.replace(/\//g,',');
                                    a.href = url; 
